...
# 时间到了结束对局
def timeOutFinish(client, cur_id):
    rm_list = []
    for room_id, room_info in client.factory.rooms.iteritems():
        if room_id != cur_id:
            if (time.time() - room_info["ready_time"]) > 5 * 60:
                msg_data = (20, 20010, -1, room_id, -1)
                buf = pack_msg(msg_data, "5i")
                for c in room_info["client_list"]:
                    c.transport.write(buf)

                rm_list.append(room_id)

    if rm_list:
        for i in rm_list:
            if i in client.factory.rooms:
                client.factory.rooms.pop(i)
    ...
    def dataReceived(self, buf):
        if buf and len(buf) >= 16:
            ...
            if len(self.factory.bufList) > MAX_BUF_LEN:
                print("堆积超出长度", len(self.factory.bufList))
